...
const char ComputerPart::PART_TYPE_NAMES[][11]{ "MONITOR", "COMPUTER", "LAPTOP", "MOUSE", "KEYBOARD", "HEADPHONES", "CAMERA" };
...
bool ComputerPart::operator==(const ComputerPart& other) const {
	if (IS_OBJ_INIT == 0 && other.IS_OBJ_INIT == 0)
		return true;
	
	return (IS_OBJ_INIT && other.IS_OBJ_INIT && 
			price == other.price &&
			type == other.type &&
			warranty == other.warranty &&
			!strcmp(brand, other.brand)
			&& !strcmp(model, other.model));
}
...
// Returns -1 if 'other' is not initialized,
//			0 if object is already initialized,
//			1 if initialization was successful.
...
void ComputerPart::print() const {
	if (IS_OBJ_INIT)
		std::cout << "{" << PART_TYPE_NAMES[type] << "|" << brand << "|" << model << "|" << warranty << "|" << price << "}";
	else
		std::cout << "{OBJECT 'ComputerPart' IS UNINITIALIZED}";
}
